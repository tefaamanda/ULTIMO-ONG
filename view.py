...
def cadastro():
    ...
    if tipo_usuario == 2:
        cur.execute("SELECT id_usuario, nome, e_mail, senha, tipo, ativo FROM usuario WHERE tipo = 2")
    elif tipo_usuario == 3:
        cur.execute("SELECT id_usuario, nome, e_mail, senha, tipo, ativo FROM usuario WHERE tipo = 3")
    elif tipo_usuario == 1:
        cur.execute("SELECT id_usuario, nome, e_mail, senha, tipo, ativo FROM usuario WHERE tipo = 1")
    ...
    for usuario in usuarios:
        ...
        if tipo == 3:
            cur.execute(
                "SELECT nome, e_mail, senha, tipo, ativo FROM usuario WHERE id_usuario = ?",
                (id_usuario,))
            ...
            cur.execute(
                "SELECT cnpj, categoria, descricao_da_causa, cep, chave_pix, num_agencia, num_conta, nome_banco, endereco, complemento, nome_resp, telefone, site_url, facebook, instagram, tipo, ativo FROM usuario WHERE id_usuario = ?",
                (id_usuario,))
